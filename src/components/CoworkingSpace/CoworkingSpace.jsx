...
import { cityAreas, priceFilterOptions, getCoworkingSpacesForCity } from './coworkingData';
import CoworkingCard from './CoworkingCard';

const CoworkingSpace = () => {
  const { cityName } = useParams();

  // Normalize city name (default to 'Gurgaon' to match design reference)
  ...
  // Available areas for current city
  const areas = useMemo(() => {
    return cityAreas[currentCity] || cityAreas['Gurgaon'] || ['All'];
  }, [currentCity]);

  // City-specific spaces
  const citySpaces = useMemo(() => {
    return getCoworkingSpacesForCity(currentCity);
  }, [currentCity]);
  ...
  const filteredSpaces = useMemo(() => {
    return citySpaces.filter((space) => {
      ...
      const matchesArea =
        selectedArea === 'All' ||
        space.area.toLowerCase().includes(selectedArea.toLowerCase()) ||
        space.address.toLowerCase().includes(selectedArea.toLowerCase());
  ...
  }, [citySpaces, selectedArea, selectedPrice]);
  ...
  return (
    <div className="w-full min-h-screen bg-white text-slate-800 pb-20 select-none">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
            Coworking Space In {currentCity}
          </h1>
        ...
        {/* Sub-Area Pills Row (Matches reference design) */}
        ...
        {/* 8 Cards Grid (4 per row on large screen, 2 on tablet, 1 on mobile) */}
      ...
      {/* Floating Call Action Button (Matches phone icon in reference) */}
      <a
